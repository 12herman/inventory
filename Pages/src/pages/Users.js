...
import { Steps, Popconfirm, Form, Modal, Space, Table, Tag, Button, Col, Row, Statistic, Divider, Input, Checkbox, Select, DatePicker, message } from 'antd';
...
import { faTrash, faPen, faInfoCircle, faLocationDot, faLandmark, faCircleCheck,faPeopleGroup} from "@fortawesome/free-solid-svg-icons"
...
const Tbdata = filterEmployeeDatas && filterEmployeeDatas.length > 0
  ...
  // Modal Save Btn
  const informationNextBtn = async () => {
  ...
  return (
    <div >
      ...
      />]}
        ...
        width={1000}
        footer={[
          saveBtn === false ? <Button key="submit" onClick={informationNextBtn}>Next</Button> : <Button key="1" onClick={ModelEditBtn} >Save</Button>,
          <Button type='text' key="2" danger="red" style={{ border: "0.5px solid red" }} onClick={() => ModelClose()}>Close</Button>
        ]}>
        
        {/* (i)Employee-Details-section */}
        {processbar.info === "process" ?<section>
        ...
        </section> : <AddressForm newempid={newempid}/>}  
